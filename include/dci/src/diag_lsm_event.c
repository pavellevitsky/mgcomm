...
SIDE EFFECTS
  None.
===========================================================================*/
void event_update_mask(unsigned char* ptr)
{
		int k;
		// populating event mask table
		for (k = 0; k < EVENT_LAST_ID/8 + 1; k++) {
			event_mask[k] = *ptr;
			ptr += 1;
		}
}
...
SIDE EFFECTS
  None.
===========================================================================*/
void event_update_dci_mask(unsigned char* ptr)
{
	if (!ptr)
		return;
	...
	memcpy(dci_cumulative_event_mask, ptr, DCI_EVENT_MASK_SIZE);
}
...
void event_report (event_id_enum_type Event_Id)
{
   if(fd != -1)
   {
      ...
      if(pEvent)
      {
		 ...
		 NumberOfBytesWritten = write(fd, (const void*) pEvent, Alloc_Len);
