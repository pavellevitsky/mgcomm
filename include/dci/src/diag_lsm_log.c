...
static int log_commit_to_cs_fail=0;
int gnDiag_Log_Initialized = 0;

#define DCI_LOG_MASK_SIZE	(16*514)
unsigned char dci_cumulative_log_mask[DCI_LOG_MASK_SIZE];
int num_dci_clients_log;

/* WM7 Log mask using shared memory */
//static HANDLE ghLog_Shared_Mask = NULL;
//byte* log_mask = NULL;
...
void log_update_mask(unsigned char* ptr)
{
	if (!ptr)
		return;
	memcpy(log_mask, ptr, LOG_MASK_SIZE);
}
...
void log_update_dci_mask(unsigned char* ptr)
{
	if (!ptr)
		return;
	...
	ptr += 4;
	memcpy(dci_cumulative_log_mask, ptr, DCI_LOG_MASK_SIZE);
}
  ...
  if (ptr)
  {
    ...
    /* Set pointer to begining of diag pkt, not the log */
      ...
      if(-1 != fd)
      {
	   ...
	   if((NumberOfBytesWritten = write(fd, (const void*) ptr, DIAG_REST_OF_DATA_POS+LOG_DIAGPKT_OFFSET + phdr_ptr->len)) != 0) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
       {
	...
	if (length > sizeof(log_header_type))
	{
		if ((enabled_mask = log_status_mask(code)) != 0) {
			...
			if (pdiag_data != NULL) {
                ...
                int NumberOfBytesWritten = 0;
				if((NumberOfBytesWritten = write(fd, (const void*) pdiag_data, DIAG_REST_OF_DATA_POS +  FPOS (diag_log_rsp_type, log) + length)) != 0) {
...
SIDE EFFECTS
  None.
===========================================================================*/
void log_set_timestamp (PACK(void *)plog_hdr_ptr)
{
	if (plog_hdr_ptr)
	{
