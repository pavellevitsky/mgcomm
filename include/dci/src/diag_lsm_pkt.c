...
=============================================================================*/
static uint16 diagpkt_get_delayed_rsp_id_lsm()
{
	uint16 delayed_rsp_id = 0;
	int err = 0;

	err = ioctl(DIAG_HANDLE, DIAG_IOCTL_GET_DELAYED_RSP_ID, &delayed_rsp_id, 0, NULL, 0, NULL, NULL);
	if (err) {
		delayed_rsp_id = 0;
...
void diagpkt_delay_commit (PACK(void *) pkt)
{
	...
	if (pkt_len > 0 && -1 != fd) {
		...
		if (is_dci_pkt) {
			...
			num_bytes = write(fd, temp, rsp_len);
			...
			num_bytes = write(fd, (const void*)pdiag_data, pkt_len + DIAG_REST_OF_DATA_POS);
		}
		if(num_bytes != 0) {
...
void diagpkt_commit (PACK(void *)pkt)
{
	...
	if ((ret = write(fd, (const void*) temp, DIAG_REST_OF_DATA_POS + rsp_length)) != 0) {
		DIAG_LOGE("Diag_LSM_Pkt: Write failed in %s, bytes written: %d, error: %d\n", __func__, ret, errno);
		gPkt_commit_fail++;
	}
	DiagSvc_Free(temp, PKT_SVC_ID);
