...
pthread_t qshrink4_copy_hdl;	/* Diag copy Qshrink4 database thread handle */
...
static char cmd_disable_log_mask[] = { 0x73, 0, 0, 0, 0, 0, 0, 0, 0xDA, 0x81, 0x7E };
static char cmd_disable_msg_mask[] = { 0x7D, 0x5D, 5, 0, 0, 0, 0, 0, 0, 0x74, 0x41, 0x7E };
static char cmd_disable_event_mask[] = { 0x60, 0, 0x12, 0x6A, 0x7E };
...
#define QS4_COPY_CMD_LEN ((2*FILE_NAME_LEN) + 10)

#define MAX_QS4_FILEPATH_LEN 512
#define MAX_QS4_FILENAME_LEN 128
#define MAX_QS4_FILENAME_ENTRIES 256 /* Based upon uint8 data type */
#define MAX_QS4_OPEN_FILES 6

static char qshrink4_dir[FILE_NAME_LEN] = "/data/qshrink4";
...
static qshrink4_cmd_struct qshrink4_cmd[NUM_PROC];
...
typedef PACK(struct) {
	...
	uint8 status;
	uint8 num_files;
	unsigned char file_entries_data;
}file_list_cmd_rsp_type;

#define FILE_LIST_CMD_CODE 75
#define FILE_LIST_SUBSYS_ID 18
#define FILE_LIST_SUBSYS_CMD_CODE 527	/* 0x20f */
#define FILE_SIZE_SIZE sizeof(uint32)
#define FILENAME_LENGTH_SIZE sizeof(uint16)
#define FILE_ENTRIES_DATA_SIZE sizeof(unsigned char)

#define QS4_VERSION 1

#define QSHRINK4_INIT 1
#define QSHRINK4_DEINIT 2

static void diag_qshrink4_init(int qs4_action)
{
	int i;
	int clear_struct = 0;
	static int qshrink4_first_init = 0;
	static int qshrink4_inited = 0;

	if (qs4_action == QSHRINK4_INIT) {
		if (!qshrink4_inited) {
			qshrink4_inited = 1;
			clear_struct = 1;
		} else {
			DIAG_LOGE("diag: In %s, QShrink4 info already initialized\n",
				__func__);
		}
	} else if (qs4_action == QSHRINK4_DEINIT) {
		if (qshrink4_inited) {
			qshrink4_inited = 0;
			clear_struct = 1;
		}
	...
	if (clear_struct) {
		for (i = 0; i< NUM_PROC; i++) {
			qshrink4_cmd[i].needs_writing = 0;
			qshrink4_cmd[i].status = -1;
			qshrink4_cmd[i].pkt_len = 0;
			if (qshrink4_first_init && qshrink4_cmd[i].cmd != NULL)
				free(qshrink4_cmd[i].cmd);
			qshrink4_cmd[i].cmd = NULL;
		}
		qshrink4_first_init = 1;
	}
}

/* Diag QShrink4 Error codes */
#define	DIAG_QS4_EPERM		1	/* Operation not permitted */
#define	DIAG_QS4_ENOENT		2	/* No such file or directory */
#define	DIAG_QS4_EINTR		3	/* Interrupted by system call */
#define	DIAG_QS4_EIO		4	/* I/O error */
#define	DIAG_QS4_EBADF		5	/* Bad file number. Fd is not a valid
					 * file descriptor or is not open for reading */
#define	DIAG_QS4_EAGAIN		6	/* Try again */
#define	DIAG_QS4_ENOMEM		7	/* Out of memory */
#define	DIAG_QS4_EACCES		8	/* Permission denied */
#define	DIAG_QS4_EFAULT		9	/* Bad address. Outside of accessible address space. */
#define	DIAG_QS4_EINVAL		10	/* Invalid argument */
#define	DIAG_QS4_EMFILE		11	/* The process already has the maximum number of files open */
#define	DIAG_QS4_ENAMETOOLONG	12	/* File name too long */
#define	DIAG_QS4_ELOOP		13	/* Too many symbolic links encountered */
#define	DIAG_QS4_EOVERFLOW	14	/* Pathname refers to a file that is too large to be opened */

uint8 get_qshrink4_error(int error)
{
	uint8 mapped_error = 0;
	switch (error)
	{
	case EPERM:
		mapped_error = DIAG_QS4_EPERM;
		break;
	case ENOENT:
		mapped_error = DIAG_QS4_ENOENT;
		break;
	case EINTR:
		mapped_error = DIAG_QS4_EINTR;
		break;
	case EIO:
		mapped_error = DIAG_QS4_EIO;
		break;
	case EBADF:
		mapped_error = DIAG_QS4_EBADF;
		break;
	case EAGAIN:
		mapped_error = DIAG_QS4_EAGAIN;
		break;
	case ENOMEM:
		mapped_error = DIAG_QS4_ENOMEM;
		break;
	case EACCES:
		mapped_error = DIAG_QS4_EACCES;
		break;
	case EFAULT:
		mapped_error = DIAG_QS4_EFAULT;
		break;
	case EINVAL:
		mapped_error = DIAG_QS4_EINVAL;
		break;
	case ENFILE:
	case EMFILE:
		mapped_error = DIAG_QS4_EMFILE;
		break;
	case ENAMETOOLONG:
		mapped_error = DIAG_QS4_ENAMETOOLONG;
		break;
	case ELOOP:
		mapped_error = DIAG_QS4_ELOOP;
		break;
	case EFBIG:
	case EOVERFLOW:
		mapped_error = DIAG_QS4_EOVERFLOW;
		break;
	default:
		mapped_error = DIAG_QS4_EIO;
		break;
...
{
	strlcpy(qshrink4_dir, dir, FILE_NAME_LEN);
	DIAG_LOGE("diag: In %s, Setting QShrink4 directory to: %s\n",
		__func__, qshrink4_dir);
}
...
static uint8 *get_qshrink4_file_list(char *qs4dir, int *pkt_len, int *cmd_status)
{
	/*
	...
	int status = 0;
	char filepath[MAX_QS4_FILEPATH_LEN];
	struct file_entry {
		uint32 file_size;
		uint16 filename_length;
		char* filename;
	...
	int num_files = 0;
	int num_file_entries = MAX_QS4_OPEN_FILES;
	...
	if (!dir_ptr) {
		if (errno == ENOENT) {
			DIAG_LOGE("diag: In %s, Warning: No QShrink4 support since directory: %s does not exist. Continuing...  \n",
				__func__, qs4dir);
		} else {
			DIAG_LOGE("diag: In %s, opendir error on directory %s, %s, errno: %d\n",
				__func__, qs4dir, strerror(errno), errno);
		}
		error_status = get_qshrink4_error(errno);
		rsp_size = sizeof(file_list_cmd_rsp_type) - FILE_ENTRIES_DATA_SIZE;

		/* Allocate memory to store the command. The calling function
		 * is responsible for freeing this memory. */
		rsp = (file_list_cmd_rsp_type *)malloc(rsp_size);
		if (rsp) {
			rsp->cmd_code = (uint8)FILE_LIST_CMD_CODE;
			rsp->subsys_id = (uint8)FILE_LIST_SUBSYS_ID;
			rsp->subsys_cmd_code = (uint16)FILE_LIST_SUBSYS_CMD_CODE;
			rsp->version = QS4_VERSION;
			rsp->status = error_status;
			rsp->num_files = num_files;
		} else {
			DIAG_LOGE("diag: In %s, unable to allocate memory for QShrink4 message\n",
				__func__);
		}
	} else {
		/* Allocate memory to hold the file information we want from the directory */
		file_entries = (struct file_entry *)malloc(num_file_entries * sizeof(struct file_entry));
		if (!file_entries) {
			DIAG_LOGE("diag: In %s, Unable to allocation memory for QShrink4 file determination\n",
				__func__);
			goto cleanup;
		}
		...
		do
		{
			...
			if (dir_entry) {
				...
				int stat_val = 0;
				int is_reg = -1;
				strlcpy(filepath, qs4dir, MAX_QS4_FILEPATH_LEN);
				(void)strlcat(filepath, dir_entry->d_name, MAX_QS4_FILEPATH_LEN);
				...
				if (is_reg) {
					...
					if (!file_entries[num_files].filename) {
						...
						num_file_entry_bytes += sizeof(uint32) + sizeof(uint16) +
		 ...
		 */
		} while (dir_entry && (num_files < MAX_QS4_FILENAME_ENTRIES));
		...
		rsp = (file_list_cmd_rsp_type *)malloc(rsp_size);
		if (!rsp) {
			DIAG_LOGE("diag: In %s, Memory allocation failure to hold QShrink4 file_list command\n",
				__func__);
			goto cleanup;
		}
		rsp->cmd_code = (uint8)FILE_LIST_CMD_CODE;
		...
		if (error_status != 0) {
			...
			for (i = 0; i < num_files; i++) {
				...
				memcpy(entry->filename, file_entries[i].filename, file_entries[i].filename_length);
...
cleanup:
		...
		if (status == -1) {
			DIAG_LOGE("In %s, closedir error, %s, errno: %d\n",
				__func__, strerror(errno), errno);
		}
	}

	*pkt_len = rsp_size;
	*cmd_status = (int)error_status;

	return (uint8 *)rsp;
}

static void get_qshrink4_info(uint32 proc_index)
{
	int status = 0;
	int pkt_len = 0;
	uint8 *cmd = NULL;
	...
	/* Check if we need to get the QShrink4 directory
	 * information for this proc_index */
	if (qshrink4_cmd[proc_index].status != -1)
		return;

	strlcpy(dir_name,qshrink4_dir,FILE_NAME_LEN);
	(void)strlcat(dir_name, proc_name[proc_index], FILE_NAME_LEN);
	(void)strlcat(dir_name, "/", FILE_NAME_LEN);

	cmd = get_qshrink4_file_list(dir_name, &pkt_len, &status);

	if (cmd && pkt_len > 0) {
		qshrink4_cmd[proc_index].status = status;
		qshrink4_cmd[proc_index].pkt_len = pkt_len;
		if (qshrink4_cmd[proc_index].cmd)
			free(qshrink4_cmd[proc_index].cmd);
		qshrink4_cmd[proc_index].cmd = cmd;
	}
}

static void *CopyQShrink4(void *ptr)
{
	DIR *qs4dir = opendir(qshrink4_dir);
	(void)ptr;

	if (!qs4dir) {
		if (errno == ENOENT) {
			DIAG_LOGE("diag: In %s, Warning: QShrink4 directory does not exist: %s. Continuing ...\n",
				__func__, qshrink4_dir);
		} else {
			DIAG_LOGE("diag: In %s, Error determining QShrink4 directory, %s, errno: %d\n",
				__func__, strerror(errno), errno);
		}
	} else {
		char qs4copycmd[QS4_COPY_CMD_LEN];
		int status = 0;

		status = closedir(qs4dir);
		if (status == -1) {
			DIAG_LOGE("diag: In %s, closedir error, %s, errno: %d\n",
				__func__, strerror(errno), errno);
		}

		std_strlprintf(qs4copycmd,
			QS4_COPY_CMD_LEN, "cp -rf %s %s/qshrink4",
			qshrink4_dir,output_dir[MSM]);

		status = system(qs4copycmd);
		if (status == -1) {
			DIAG_LOGE("diag: In %s, Error copying QShrink4 directory, %s, errno: %d\n",
				__func__, strerror(errno), errno);
		}
	}
...
void flush_buffer(int signal)
{
	...
	char pid_file[DIAG_MDLOG_PID_FILE_SZ] = DIAG_MDLOG_PID_FILE;
	int success = 0;
	...
	if (signal) {
		...
		if (stat(pid_file, &pid_stat) == 0)
			...
			DIAG_LOGE("diag: Unable to delete the PID file, err: %d\n", errno);
...
static void process_diag_payload(void)
{
	...
	printf("---------------------------process_diag packet type: %d  \n", type);
	if(type == MSG_MASKS_TYPE) {
		...
		if (num_bytes_read < 4)
			return;
		diagpkt_LSM_process_request((void*)ptr, (uint16)num_bytes_read - 4, type);
	}
...
void log_to_device(unsigned char *ptr, int logging_mode, int size, int type)
{
		...
		if(logging_mode == MEMORY_DEVICE_MODE) {
			...
			if (fd_md[type] < 0) {
				...
				DIAG_LOGE(" creating new file %s \n",
							file_name_curr[type]);
				file_count++;
				/* The qshrink4 file list command needs to be
				 * the first command written in the file */
				if (qshrink4_cmd[type].pkt_len > 0 &&
					qshrink4_cmd[type].cmd)
					qshrink4_cmd[type].needs_writing = 1;
			}
	...
	if (fd_dev != -1) {
		if(logging_mode == MEMORY_DEVICE_MODE ){
			if (qshrink4_cmd[type].needs_writing) {
				ret = write(fd_dev, (const void*)qshrink4_cmd[type].cmd,
						qshrink4_cmd[type].pkt_len);
				if (ret > 0 ) {
					count_written_bytes[type] +=
						qshrink4_cmd[type].pkt_len;
					qshrink4_cmd[type].needs_writing = 0;
...
int diag_load_mask(unsigned char* buf, int len)
{
    ...
    *(int *)mask_buf = USER_SPACE_DATA_TYPE;
    ...
    get_qshrink4_info((uint32)proc_type);
	get_circular_logging_info(proc_type);
    
    for (i=0; i<len; i++){
        if ((index > 0) && (count_mask_bytes == 4)) {
            *(int *)(mask_buf + count_mask_bytes) = token_list[index];
			count_mask_bytes = 8;
		}
		mask_buf[count_mask_bytes] = buf[i];
...
void diag_switch_logging(int requested_mode, char *dir_location_msm)
{
	...
	int mode = requested_mode;
		...
		if(mode == MEMORY_DEVICE_MODE){
			...
			pthread_create( &qshrink4_copy_hdl, NULL, CopyQShrink4, NULL);
			if (qshrink4_copy_hdl == 0) {
				DIAG_LOGE("diag: In %s, Failed to create copy qshrink4 thread",
					__func__);
			}
			log_to_memory = 1;
		...
		if ((err = ioctl(fd, DIAG_IOCTL_SWITCH_LOGGING, &mode, 0, NULL, 0, NULL, NULL)) < 0) {
			DIAG_LOGE(" diag: DeviceIOControl failed while switching to log mode: %d, error: %d\n",
					mode, errno);
		} else {
			if (logging_mode == MEMORY_DEVICE_MODE) {
...
int diag_get_real_time_status_proc(int proc, int *real_time)
{
	...
	query.proc = proc;
	err = ioctl(fd, DIAG_IOCTL_GET_REAL_TIME, &query, 0, NULL, 0, NULL, NULL);
	if (err != 0) {
...
boolean Diag_LSM_Init (byte* pIEnv)
{
   ...
   if (DIAG_HANDLE == DIAG_INVALID_HANDLE) {
	...
	for (i = 0; i< NUM_PROC; i++) {
      ...
      diag_qshrink4_init(QSHRINK4_INIT);
...
boolean Diag_LSM_DeInit (void)
{
   ...
   if( !Diag_LSM_Pkt_DeInit() || !Diag_LSM_Msg_DeInit())
    ...
    fd = DIAG_INVALID_HANDLE;
     ...
     diag_qshrink4_init(QSHRINK4_DEINIT);
