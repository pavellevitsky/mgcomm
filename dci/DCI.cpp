...
DCI::~DCI()
{
    std::cout << __func__ << " -> releasing DCI interface\n";
...
int DCI::setup_interface()
{
    ...
    */
    ...
    std::cout << __func__ << " -> setup DCI interface -> ";
    ...
    {
        DIAG_LOGE("could not register with Diag LSM -> errno:%d\n", errno);
        return -1;
    }
    ...
    */
    ...
    if (err != DIAG_DCI_NO_ERROR) {
        DIAG_LOGE("failed register with DCI -> err:%d errno:%d\n", err, errno);
        return -1;
    } else
        DIAG_LOGE("registered with DCI -> client-id:%d\n", _client_id);
...
int DCI::_dci_configure_logs(int action, std::unordered_set<uint16_t> codes)
{
    ...
    int result =  diag_log_stream_config(_client_id, action, log_codes_vector.data(), log_codes_vector.size());
...
int DCI::_dci_configure_events(int action, std::unordered_set<int> codes)
{
    ...
    int result = diag_event_stream_config(_client_id, action, event_codes_vector.data(), event_codes_vector.size());
